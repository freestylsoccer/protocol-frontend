...
import ProtocolDataProvider from '../blockchain/contracts/misc/AaveProtocolDataProvider.sol/AaveProtocolDataProvider.json'
...
export default function GoodsExport ({ requestAccount, account }) {
        ...
        if (typeof window.ethereum !== 'undefined' && account !== undefined) {
            const init = async () => {
                ...
                try {
                    ...
                    const addressDataProvider = "0x3c1cCF441c10Cb61aC94B5099315d3c9D8BA3d8a"
                    ...
                    // const res = await dataProvider.getReserveData("0x080De04372D5f1E317d492645b923f6286C7eC86")
                    const res = await lendingPool.getReserveData("0x080De04372D5f1E317d492645b923f6286C7eC86")
                    console.log(res)
    ...
    const withdraw = async (e) => {
      e.preventDefault();
      var amt = ethers.utils.parseUnits("988501", 6)
      console.log(amt)
      try {
        await lendingPool.withdraw("0x080De04372D5f1E317d492645b923f6286C7eC86", amt, account)
        console.log('done')
      } catch (err) {
        console.log(err.message);
        setMessage(err.message);
                            ...
                            {!finish &&
                                <div className="min-h-screen flex items-center justify-center bg-gradient-to-b py-2 from-black to-gray-900 px-4 sm:px-6 lg:px-8">
                                    <div className="max-w-md w-full space-y-8">
                                        ...
                                        <form className="mt-8 space-y-6" onSubmit={withdraw}>
                                            <input type="hidden" name="remember" defaultValue="true" />
                                            <div className="rounded-md shadow-sm -space-y-px">
                                                <div>
                                                <label htmlFor="amount">
